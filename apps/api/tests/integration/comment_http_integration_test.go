...
func TestCommentHTTPCreateThreadModerationLifecycleFlow(t *testing.T) {
	...
	restoreRec := performCommentJSONRequest(t, handler, http.MethodPost, "/comments/"+rootRes.CommentID+"/restore", map[string]any{
		"actor_user_id": authorRoot,
	})
...
func TestCommentHTTPCreateValidationFailure(t *testing.T) {
	...
	invalidCreateRec := performCommentJSONRequest(t, handler, http.MethodPost, "/comments", map[string]any{
		...
		"author_user_id": "not-a-uuid",
		"content":        "",
	})
	require.Equal(t, http.StatusBadRequest, invalidCreateRec.Code)
}

func performCommentJSONRequest(t *testing.T, handler http.Handler, method string, path string, body any) *httptest.ResponseRecorder {
	t.Helper()

	payload, err := json.Marshal(body)
	require.NoError(t, err)

	return performCommentRequest(t, handler, method, path, bytes.NewReader(payload))
}

func performCommentRequest(t *testing.T, handler http.Handler, method string, path string, body *bytes.Reader) *httptest.ResponseRecorder {
	t.Helper()
